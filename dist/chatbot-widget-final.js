...
(function () {
    ...
    class HKUChatbotWidget {
        constructor(options = {}) {
            this.widget = null;
            this.elements = {};
            const globalConfig = window.HKUChatbotConfig
                ? window.HKUChatbotConfig.getEnvironmentConfig()
                : {};
            this.options = Object.assign({
                ...
                showWelcome: options.showWelcome !== undefined
                    ? options.showWelcome
                    : globalConfig.showWelcome !== undefined
                        ? globalConfig.showWelcome
                        : true,
            }, options);
            this.config = window.HKUChatbotConfig || null;
            this.isOpen = false;
        ...
        init() {
            ...
            if (this.options.debug) {
                console.log("HKU Chatbot Widget initialized with options:", this.options);
      ...
      `;
            ...
            this.elements = {
                ...
                scrollToBottomBtn: widget.querySelector("#hku-chatbot-scroll-to-bottom"),
        ...
        bindEvents() {
            ...
            this.elements.fullscreenButton.addEventListener("click", () => this.openFullscreen());
            ...
            document.addEventListener("mouseup", (e) => {
                var _a, _b, _c, _d, _e;
                if (!this.isOpen || !mouseDownTarget)
                    return;
                ...
                const hasSelection = ((_a = window.getSelection()) === null || _a === void 0 ? void 0 : _a.toString().length) > 0;
                const isLongPress = timeDiff > 300; // 300ms threshold
                const isDragAction = mouseDownTarget !== mouseUpTarget &&
                ...
                const isMouseDownOutside = !((_b = this.elements.container) === null || _b === void 0 ? void 0 : _b.contains(mouseDownTarget)) &&
                    !((_c = this.elements.toggle) === null || _c === void 0 ? void 0 : _c.contains(mouseDownTarget));
                const isMouseUpOutside = !((_d = this.elements.container) === null || _d === void 0 ? void 0 : _d.contains(mouseUpTarget)) &&
                    !((_e = this.elements.toggle) === null || _e === void 0 ? void 0 : _e.contains(mouseUpTarget));
        ...
        openChat() {
            ...
            if (this.elements.messages.children.length === 0 &&
                ...
                const welcomeMessage = this.config && this.config.getMessage
        ...
        sendMessage() {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    const response = yield fetch(this.options.chatUrl, {
                        method: "POST",
                        headers,
                    ...
                    if (botResponse.includes("Please wait. Type 'Cancel' to cancel the queue")) {
                        if (this.options.debug) {
                    ...
                    this.hideTypingIndicator();
                    const errorMessage = this.config && this.config.getMessage
                        ? this.config.getMessage("error") ||
        ...
        addMessage(text, sender) {
            ...
            if (sender !== "human") {
                ...
                copyButton.classList.add("hku-chatbot-action-button", "hku-chatbot-copy-button");
                copyButton.setAttribute("aria-label", sender === "user" ? "Copy message to input" : "Copy");
          ...
          <path d="M16 1H4C3 1 2 2 2 3v14h2V3h12V1zm3 4H8C7 5 6 6 6 7v14c0 1 1 2 2 2h11c1 0 2-1 2-2V7c0-1-1-2-2-2zm0 16H8V7h11v14z"/>
                ...
                if (sender === "bot") {
                    ...
                    likeButton.classList.add("hku-chatbot-action-button", "hku-chatbot-like-button");
            ...
            <path d="M9 21h9c.83 0 1.54-.5 1.84-1.22l3.02-7.05c.09-.23.14-.47.14-.73v-2c0-1.1-.9-2-2-2h-6.31l.95-4.57.03-.32c0-.41-.17-.79-.44-1.06L14.17 1 7.58 7.59C7.22 7.95 7 8.45 7 9v10c0 1 1 2 2 2zM9 9l4.34-4.34L12 10h9v2l-3 7H9V9zM1 9h4v12H1V9z"/>
                    ...
                    dislikeButton.classList.add("hku-chatbot-action-button", "hku-chatbot-dislike-button");
            ...
            <path d="M15 3H6c-.83 0-1.54.5-1.84 1.22l-3.02 7.05c-.09.23-.14.47-.14.73v2c0 1.1.9 2 2 2h6.31l-.95 4.57-.03.32c0 .41.17.79.44 1.06L9.83 23l6.59-6.59c.36-.36.58-.86.58-1.41V5c0-1.1-.9-2-2-2zm0 12l-4.34 4.34L12 14H3v-2l3-7h9v10zm3-12h4v12h-4V3z"/>
                    ...
                    likeButton.addEventListener("click", (e) => __awaiter(this, void 0, void 0, function* () {
                        e.stopPropagation(); // Prevent event bubbling
                        ...
                        if (likeButton.classList.contains("active")) {
                            try {
                                yield fetch(this.options.chatUrl, {
                                    method: "POST",
                    ...
                    dislikeButton.addEventListener("click", (e) => __awaiter(this, void 0, void 0, function* () {
                        e.stopPropagation(); // Prevent event bubbling
                        if (likeButton.classList.contains("active")) {
        ...
        parseMarkdownInline(text) {
            ...
            // Inline code
            html = html.replace(/`([^`]+)`/g, '<code class="hku-inline-code">$1</code>');
            // Auto-link URLs (avoid double-linking)
            html = html.replace(/(?<!href=["']|src=["'])(?<!<a[^>]*>.*?)(https?:\/\/[^\s<>"]+)(?![^<]*<\/a>)/g, '<a href="$1" target="_blank" rel="noopener noreferrer">$1</a>');
            return html;
        }
        parseMarkdownTables(text) {
            ...
            return text.replace(tableRegex, (match, header, separator, rows) => {
                const headerCells = header
                    .split("|")
                    .slice(1, -1)
                    .map((cell) => `<th class="hku-table-header">${this.parseMarkdownInline(cell.trim())}</th>`)
                    .join("");
                const rowsHtml = rows
                    .trim()
                    .split("\n")
                    ...
                    const cells = row
                        .split("|")
                        .slice(1, -1)
                        .map((cell) => `<td class="hku-table-cell">${this.parseMarkdownInline(cell.trim())}</td>`)
                        .join("");
        ...
        smartScroll(sender, messageElement) {
            if (sender === "user") {
                ...
                const allMessages = Array.from(this.elements.messages.getElementsByClassName("hku-chatbot-message-wrapper"));
                ...
                for (let i = allMessages.length - 2; i >= 0; i--) {
                    ...
                    if (allMessages[i].classList.contains("hku-chatbot-user-wrapper")) {
        ...
        scrollToUserMessage(messageElement, smooth = true) {
            if (!messageElement)
                return;
            setTimeout(() => {
        ...
        startPolling() {
            if (!this.pollingInterval) {
                ...
                this.pollingInterval = window.setInterval(() => this.pollForMessages(), 1000);
            ...
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    if (message && message !== "-") {
                        ...
                        else if (message.includes("Please wait. Type 'Cancel' to cancel the queue")) {
                            // Don't show "Please wait" message, just keep connecting indicator visible
                            console.log("Received queue message during polling, keeping indicator visible");
        ...
        openFullscreen() {
            try {
                ...
                if (newTab) {
                    newTab.focus();
                    if (this.options.debug) {
                        console.log("HKU Chatbot Widget: Opened fullscreen chatbot in new tab");
                    }
                }
                else {
                    if (this.options.debug) {
                        console.warn("HKU Chatbot Widget: Popup blocked, trying to navigate in current tab");
                    }
                    const popupBlockedMessage = this.config && this.config.getMessage
                        ...
                        : "Popup was blocked by your browser. Please allow popups for this site or click here to open fullscreen:";
                    this.addMessage(`${popupBlockedMessage} <a href="${fullscreenUrl}" target="_blank" rel="noopener noreferrer">Open Fullscreen Chatbot</a>`, "bot");
                }
            }
            catch (error) {
                console.error("HKU Chatbot Widget: Error opening fullscreen chatbot:", error);
                this.addMessage(`Error opening fullscreen view. Please visit: <a href="${this.options.fullscreenUrl}" target="_blank" rel="noopener noreferrer">Fullscreen Chatbot</a>`, "bot");
            }
        }
        sendDisconnectMessage() {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    if (this.options.debug) {
                        console.log("Disconnect message sent, response status:", response.status);
                    }
                }
                catch (error) {
                    if (this.options.debug) {
                        console.error("HKU Chatbot Widget: Error sending disconnect message with fetch:", error);
                    }
                    ...
                    try {
                        ...
                        if (this.options.debug) {
                            console.error("HKU Chatbot Widget: Error sending disconnect message with sendBeacon:", beaconError);
                        }
                    }
                }
            });
    ...
    // Global API
    window.HKUChatbotWidget = HKUChatbotWidget;
